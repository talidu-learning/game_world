...
#pragma warning restore CS0414
    ...
    [SerializeField] [HideInInspector] private bool hitProps;

    [SerializeField] [ToggleLeft] private bool hit3DObjects = true;

    [SerializeField] [ToggleLeft] private bool hit2DObjects = true;

    [SerializeField] [ToggleLeft] private bool hitWorldSpaceUI = true;

    [SerializeField] [ToggleLeft] private bool hitScreenSpaceUI = true;

    [SerializeField] private LayerMask layerMask = -1;

    [SerializeField] [ToggleLeft] private bool useHitFilters = false;
        ...
        if (hitWorldSpaceUI)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if ((canvas == null) || (canvas.renderMode == RenderMode.ScreenSpaceOverlay) ||
                    (canvas.worldCamera != _camera)) continue;
    ...
    private void performUISearchForCanvas(IPointer pointer, Canvas canvas, GraphicRaycaster raycaster,
        Camera eventCamera = null, float maxDistance = float.MaxValue, Ray ray = default(Ray))
    {
        var position = pointer.Position;
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (graphic.Raycast(position, eventCamera))
            {
                ...
                else
                {
                    ...
                    distance = Vector3.Dot(transForward, t.position - ray.origin) /
